...
def get_arguments():
    parser = argparse.ArgumentParser(description="Unet3D_CQTr")
    # parameters for path
    ...
    parser.add_argument("--snapshot_dir", type=str, default='snapshots/CQTr/')
        ...
        for epoch in range(args.num_epochs):
            ...
            if (epoch >= 0) and (args.local_rank == 0) and (
                ...
                torch.save(model.state_dict(), osp.join(args.snapshot_dir,
                                                        'MOTS_CQTr_' + args.snapshot_dir.split('/')[-2] + '_e' + str(
                                                            epoch) + '.pth'))

            if (epoch >= args.num_epochs - 1) and (args.local_rank == 0):
                print('save model ...')
                torch.save(model.state_dict(), osp.join(args.snapshot_dir, 'MOTS_CQTr_' + args.snapshot_dir.split('/')[
